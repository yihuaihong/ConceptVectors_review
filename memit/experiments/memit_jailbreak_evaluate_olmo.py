...
def jailbreak_evaluate(model, tokenizer, Concept):
    ...
    for j in range(0, 4):
        ...
        x[f'QA-JB model answers memit-{j}'] = outputs
    ...
    outputs = tokenizer.batch_decode(generation_output[:, -n_new_tokens:], skip_special_tokens=True)
    x[f'QA-JB unrelated_qa model answers memit'] = outputs

    with open(join(data_dir, f"olmo_concepts_with_jb_answers_{x['Concept']}_memit_entropy.json"), 'w') as f:
...
def main(
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        jailbreak_evaluate(model=edited_model, tokenizer=tok, Concept=concepts_list[order])
